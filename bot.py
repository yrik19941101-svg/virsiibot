...
class HeikenAshiBot:
    def __init__(self, config):
        ...
        self.state = {}  # для каждой монеты храним последнюю обработанную свечу и флаг отправки
    ...
    async def process_symbol(self, symbol):
        ...
        current_timestamp = df['timestamp'].iloc[-1]  # метка текущей незакрытой свечи

        if symbol not in self.state:
            self.state[symbol] = {
                'last_timestamp': None,
                'signal_sent_for_current': False
            }
        state = self.state[symbol]

        # Если появилась новая свеча (таймштамп изменился) — сбрасываем флаг отправки
        if current_timestamp != state['last_timestamp']:
            state['last_timestamp'] = current_timestamp
            state['signal_sent_for_current'] = False

        # Если сигнал для этой свечи уже был отправлен — выходим
        if state['signal_sent_for_current']:
            return
        ...
        if prev2_color == 'red' and prev1_color == 'green':
            if current_candle['low'] < current_ha_open:
                ...
                state['signal_sent_for_current'] = True
