...
class HeikenAshiBot:
    ...
    async def load_symbols(self):
        ...
        await bot.send_message(
            ...
            text=f"✅ Бот запущен (CoinGecko, с логами)\n"
    ...
    async def get_market_data(self, coin_id, limit=30):
        try:
            logger.info(f"Запрашиваю данные для {coin_id}...")
            ...
            df = df.tail(limit)
            logger.info(f"Получено {len(df)} свечей для {coin_id}")
            return df
    ...
    async def process_symbol(self, coin_id):
        logger.info(f"🔍 Начинаю проверку монеты: {coin_id}")
        df = await self.get_market_data(coin_id, limit=30)
        if df is None or len(df) < 10:
            logger.warning(f"Недостаточно данных для {coin_id}, пропускаю")
            return
        ...
        if current_timestamp != state['last_timestamp']:
            ...
            logger.info(f"Новая свеча для {coin_id}: {current_timestamp}")

        if state['signal_sent']:
            logger.debug(f"Сигнал для {coin_id} уже отправлен на этой свече, пропускаю")
            return
        ...
        if prev2_color == 'red' and prev1_color == 'green':
            if current_candle['low'] < current_ha_open:
                ...
                logger.info(f"LONG сигнал отправлен для {coin_id}")
        # SHORT
            ...
            if current_candle['high'] > current_ha_open:
                ...
                state['signal_sent'] = True
                logger.info(f"SHORT сигнал отправлен для {coin_id}")
        else:
            logger.debug(f"Условия сигнала не выполнены для {coin_id}")
        ...
        while True:
            for coin_id in self.all_symbols:
                try:
                    ...
                    logger.error(f"Ошибка при обработке {coin_id}: {e}")
                await asyncio.sleep(1.5)  # пауза между монетами
